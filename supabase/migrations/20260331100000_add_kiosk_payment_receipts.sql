...
--
-- Stores per-employee monthly payment receipts that employees review and sign
-- on the kiosk. Each receipt captures a snapshot of the settlement data at
-- signing time, an employee signature image, and a rendered PDF/JSON document.
--
-- The partial unique index allows superseded receipts (corrections) while
-- enforcing at most one active (pending | signed) receipt per employee/month.
--
-- Storage buckets hold receipt signature images and rendered documents.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- 3. Partial unique index: only one active receipt per employee/month
