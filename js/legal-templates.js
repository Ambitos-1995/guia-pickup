var LegalTemplates = (function () {
    'use strict';

    var CURRENT_TEMPLATE_VERSION = 'legal_v2_2026_03_31';
    var LEGACY_CONTRACT_TEMPLATE_VERSION = 'legacy_contract_v1';
    var LEGACY_RECEIPT_TEMPLATE_VERSION = 'legacy_receipt_v1';

    function clone(value) {
        return JSON.parse(JSON.stringify(value));
    }

    function buildContractOpening(employeeName) {
        return 'Entre la Fundacion Ambitos («la Fundacion») y ' + (employeeName || '—') + ' («el/la Participante»), ambas partes acuerdan lo siguiente:';
    }

    function buildLegacyContractContent(employeeName) {
        return {
            schema_version: 1,
            ...
            clauses: [
                ...
                },
                {
                    ...
                    text: 'La participacion es completamente voluntaria. Cualquiera de las partes puede dar por finalizado este contrato en cualquier momento, sin necesidad de preaviso ni penalizacion.'
                },
                {
                ...
                },
                {
                    ...
                    text: 'Este contrato tiene una duracion de tres meses desde la fecha de firma, renovable automaticamente por periodos iguales salvo comunicacion en contrario por cualquiera de las partes.'
                }
            ],
    ...
    function buildCurrentContractContent(employeeName) {
        return {
            schema_version: 2,
    ...
    function buildLegacyReceiptContent() {
        return {
            schema_version: 1,
            ...
            intro_text: 'La persona participante declara haber revisado este documento antes de firmarlo y confirma que los datos mostrados corresponden a su actividad ocupacional en el Punto de Entrega SEUR - Punto Inclusivo.',
            confirmation_text: 'Con tu firma electronica confirmas haber recibido la gratificacion indicada, correspondiente a las horas efectivamente realizadas durante el periodo mostrado, dentro del programa ocupacional regulado por el Real Decreto 2274/1985.',
            pdf_confirmation_text: 'El/la participante confirma haber recibido la gratificacion indicada, correspondiente a su participacion en la actividad ocupacional del Punto de Entrega SEUR - Punto Inclusivo, conforme al Real Decreto 2274/1985.'
        };
    }

    function buildCurrentReceiptContent() {
        return {
            schema_version: 2,
            template_version: CURRENT_TEMPLATE_VERSION,
            ...
            intro_text: 'La persona participante declara haber revisado este documento antes de firmarlo y confirma que los datos mostrados corresponden a su participacion voluntaria en el programa de rehabilitacion psicosocial \'Punto Inclusivo\' de Fundacion Ambitos.',
            confirmation_text: 'Con tu firma electronica confirmas haber recibido la gratificacion indicada, correspondiente a tu participacion voluntaria en el programa de rehabilitacion psicosocial \'Punto Inclusivo\' de Fundacion Ambitos. Esta cantidad tiene caracter de gratificacion terapeutico-ocupacional y no constituye salario ni retribucion laboral, de acuerdo con los fines fundacionales de la entidad (Ley 50/2002, de 26 de diciembre) y el marco de derechos de las personas con discapacidad (Real Decreto Legislativo 1/2013, de 29 de noviembre).',
            pdf_confirmation_text: 'El/la participante confirma haber recibido la gratificacion indicada, correspondiente a su participacion voluntaria en el programa de rehabilitacion psicosocial \'Punto Inclusivo\' de Fundacion Ambitos. Esta cantidad tiene caracter de gratificacion terapeutico-ocupacional y no constituye salario ni retribucion laboral, de acuerdo con los fines fundacionales de la entidad (Ley 50/2002, de 26 de diciembre) y el marco de derechos de las personas con discapacidad (Real Decreto Legislativo 1/2013, de 29 de noviembre).'
        };
    }

    function resolveContractContent(snapshot, employeeName) {
        if (snapshot && snapshot.rendered_content && snapshot.rendered_content.clauses) {
            return clone(snapshot.rendered_content);
        }
        return buildLegacyContractContent(employeeName);
    }

    function resolveReceiptContent(snapshot) {
        if (snapshot && snapshot.rendered_content && snapshot.rendered_content.pdf_confirmation_text) {
            return clone(snapshot.rendered_content);
        }
        return buildLegacyReceiptContent();
    }

    return {
        CURRENT_TEMPLATE_VERSION: CURRENT_TEMPLATE_VERSION,
        buildCurrentContractContent: buildCurrentContractContent,
        buildLegacyContractContent: buildLegacyContractContent,
        resolveContractContent: resolveContractContent,
        buildCurrentReceiptContent: buildCurrentReceiptContent,
        buildLegacyReceiptContent: buildLegacyReceiptContent,
        resolveReceiptContent: resolveReceiptContent
    };
}());
